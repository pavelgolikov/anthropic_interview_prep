...
**How this was made:** the five mock specs and their test suites were generated to
mirror CodeSignal's published Industry Coding Framework format — one system, built
across five progressive levels, with the visible tests as the executable spec. Every
solution under `mocks/` is mine, written against a 90-minute timer with no assistance.
`reference/` holds an independent worked solution for each, used only for post-mortems.
The mock tests themselves were generated by Claude Code.

Four of the five mocks were solved twice, on separate days: `first_pass.py` and
`solution.py` are independent attempts at the same spec, and both pass all levels.
`m5_filesystem` was solved once.
...
3. No AI assistance of any kind during a run. Only <https://docs.python.org/3/>, same as the real assessment.
